...
# ✅ FOTO KAMU (PASTI MUNCUL DI SEMUA PESAN)
...
# ==================== FUNGSI PESAN ✅ SELALU PAKAI FOTO ====================
def kirim_pesan(chat_id, teks, tombol=None):
    try:
        # ✅ SETIAP KIRIM PESAN PASTI TAMPILKAN FOTO
        bot.send_photo(
            chat_id=chat_id,
            photo=FOTO_URL,
            caption=teks,
...
# ==================== KIRIM EMAIL ✅ FOTO TETAP MUNCUL SAAT PROSES ====================
def kirim_email_proses(id_user, pengirim, sandi, tujuan, isi, subjek):
    # ✅ Notifikasi awal pakai foto
...
📤 Dari: `{pengirim}`""",
    ...
    bot.edit_message_caption(
        id_user, pesan.message_id,
        "❌ **SEMUA JALUR GAGAL**\nRailway Gratis memblokir port email. Gunakan Render/Termux ya!",
        parse_mode="Markdown",
        reply_markup=menu_utama(id_user)
    )
    return False
...
# ==================== PERINTAH BOT ✅ SEMUA TAMPIL FOTO ====================
...
def inputan(msg):
    ...
    if l == "kode":
        if msg.text.strip() == SANDI_UTAMA:
            ...
            kirim_pesan(id, "✅ **BERHASIL MASUK!**\nSelamat menggunakan layanan premium.", menu_utama(id))
        else:
            kirim_pesan(id, "❌ Kode akses salah! Silakan coba lagi.")

    elif l == "input_email":
        proses[id]["email"] = msg.text.strip()
        proses[id]["langkah"] = "input_sandi"
        kirim_pesan(id, "📩 Masukkan alamat Gmail yang ingin ditambahkan:")

    elif l == "input_sandi":
        a = baca_akun()
        a.append({"email":proses[id]["email"], "sandi":msg.text.strip()})
        simpan_akun(a)
        kirim_pesan(id, f"✅ **BERHASIL DITAMBAHKAN**\nAkun: `{proses[id]['email']}`", menu_utama(id))

    elif l == "input_nomor":
        nomor = cek_nomor(msg.text)
        if not nomor:
            kirim_pesan(id, "❌ Format salah! Contoh: `+6281234567890`")
            return
        a = baca_akun()
        if not a:
        ...
        kirim_email_proses(id, g["email"], g["sandi"], TUJUAN_EMAIL,
            f"Saya mengajukan banding pemulihan akun untuk nomor {nomor}. Mohon bantu verifikasi, terima kasih banyak.",
...
def tombol(c):
    ...
    if not cek_akses(id, uname):
        bot.answer_callback_query(c.id, "❌ Kamu tidak punya izin akses!")
        return
...
if __name__ == "__main__":
    print("🚀 BOT PREMIUM ASEP KANJUT SIAP BERJALAN!")
    print("✅ FOTO MUNCUL DI SEMUA PESAN & MENU")
    while True:
        try:
